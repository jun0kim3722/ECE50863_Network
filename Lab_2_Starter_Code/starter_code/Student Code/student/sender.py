...
class sender():
	def __init__(self, send_monitor, TRANS_DELAY, TIMEOUT, receiver_id, data_list, max_packet_size, window_size,
		...
		self.max_window_size_ = window_size * 5
	...
	def send_packets(self):
		...
		while self.missing_packets_:
			...
			while end_idx < len(self.missing_packets_):
				...
				# print(f"send range: {start_idx} : {end_idx - 1}", len(self.missing_packets_))
				for packet_idx in self.missing_packets_[start_idx : end_idx]:
					...
					# print("send", packet_idx, end_idx, len(self.missing_packets_) - 1)
	...
	def window_ctr(self):
		...
		while self.missing_packets_:
			try:
				...
				print(f'Sender: Got response for {data}/{len(self.data_list_)}. Window size: {self.window_size_}')
			except:
			...
			if self.eow_flag:
				...
				adjustment = self.kp_ * error + self.ki_ * self.integral_ + self.kd_ * derivative
				self.previous_error_ = error

				# update window size
				self.window_size_ = max(10, int(self.window_size_ + adjustment), self.max_window_size_)
				self.eow_flag = False
...
if __name__ == '__main__':
	...
	sd = sender(send_monitor, TRANS_DELAY, TIMEOUT, receiver_id, data_list, max_packet_size, window_size,
			  	target_ratio=0.05, kp=1.0, ki=0.1, kd=0.05)
