...
def calc_weighted_qoe(qualities, bitrates, buffer_init, previous_throughput, seconds_per_chunk, q_coef, v_coef, r_coef, weight):
	qoe = 0.0
	buffer = buffer_init
	rebuffer = 0.0
	prev_quality = None
	...
	for i, q in enumerate(qualities):
		...
		download_time = size_kB / estimate_bandwidth(previous_throughput)
...
def student_entrypoint(client_message: ClientMessage):
	...
	weight = [chuck_weight(c) for c in lookahead_bitrates] # prefer cheaper high qualities
	...
	for seq in product(range(client_message.quality_levels), repeat=len(lookahead_bitrates)):
		...
		qoe = calc_weighted_qoe(
				...
				client_message.quality_coefficient,
				client_message.variation_coefficient,
