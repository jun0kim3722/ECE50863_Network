...
def calc_bba2(buffer_sec, quality_levels):
	if buffer_sec <= RESERVOIR:
		return 0
	elif buffer_sec >= RESERVOIR + CUSHION:
		return quality_levels - 1
	else:
		fraction = (buffer_sec - RESERVOIR) / CUSHION
		return int(fraction * (quality_levels - 1))
...
def student_entrypoint(client_message: ClientMessage):
	...
	q_level = calc_bba2(buffer_sec, client_message.quality_levels)
	return q_level