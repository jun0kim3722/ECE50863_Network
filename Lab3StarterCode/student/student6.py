...
CUSHION = 10.0
THROUGHPUT_HISTORY = []
RECENT_REBUFFER_FLAG = False  # Global tracker for whether a rebuffer recently occurred
...
def compute_qoe_sequence(qualities, bitrates, buffer_init, bandwidth, spc, q_coef, v_coef, r_coef, smart_bias, confidence_penalty):
    ...
    for i, q in enumerate(qualities):
        ...
        buffer += spc
        qoe += q * q_coef + smart_bias[i][q] * 0.5
    ...
    RECENT_REBUFFER_FLAG = rebuffer > 0.01  # if even slight rebuffering occurred, enter cautious mode
    return qoe
...
def student_entrypoint(client_message: ClientMessage):
	...
	if buffer_sec < CUSHION or RECENT_REBUFFER_FLAG or confidence_penalty > 0.2:
		bandwidth_est *= 0.8
